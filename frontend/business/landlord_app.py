...
async def main(page: ft.Page):
            ...
            match event["type"]:
                case "auction-completed":
                    ...
                    page.snack_bar = ft.SnackBar(
                        content=ft.Text(
                            f"{event['agent'].localpart} completed with final price: {final_price}"
                        )
                    )
    ...
    required_note = ft.Text(
        "* indicates required fields", size=12, color=ft.colors.GREY_700, italic=True
    )
    ...
    city = ft.TextField(
        label="City (optional)", hint_text="Enter city", width=190, value="Warszawa"
    )
    ...
    location_buttons = ft.Row(
        [
            ...
            ft.ElevatedButton(
                "View on Map", on_click=lambda _: open_map(), icon=ft.icons.MAP
            ),
        ]
    )

    submit_button = ft.ElevatedButton(
        "Add Listing", on_click=lambda _: submit_form(), width=200, disabled=True
    )
    ...
    form_view = ft.Container(
        content=ft.Column(
            [
                ...
                address_error_text,
                ft.Row(
                    [verified_apartment, verified_postal],
                    alignment=ft.MainAxisAlignment.START,
                ),
                ft.Row(
                    [verified_voivodeship, verified_country],
                    alignment=ft.MainAxisAlignment.START,
                ),
                ft.Row(
                    [
    ...
    tab_content = ft.Container(content=tabs_content[0], padding=ft.padding.only(top=20))
    ...
    def validate_form():
        ...
        submit_button.disabled = not all(
            [name_valid, address_valid, price_valid, area_valid, checkbox_valid]
        )
        page.update()
        ...
        try:
            ...
            if full_address:
                ...
                if location_data:
                    ...
                    if (
                        not address_components[0][0].isdigit()
                        and address_components[1][0].isdigit()
                    ):
                        address_components = address_components[1:]

                    verified_street.value = ""
                    verified_apartment.value = (
                        local_number.value if local_number.value else ""
                    )
                    ...
                    if address_components[0]:
                        verified_street.value = (
                            address_components[1] + " " + address_components[0]
                        )
                    ...
                    page.overlay.append(
                        ft.SnackBar(content=ft.Text("Location not found!"))
                    )
                    page.update()
        except Exception as e:
            page.overlay.append(
                ft.SnackBar(content=ft.Text(f"Error searching location: {str(e)}"))
            )
            page.update()
    ...
    def submit_form():
        ...
        if not all(
            ...
            page.overlay.append(
                ft.SnackBar(
                    content=ft.Text("Please correct all errors before submitting")
                )
            )
            return
                ...
                content=ft.Container(
                    content=ft.Column(
                        [
                            ...
                            ft.Text(
                                (
                                    f"Blacklisted premises: {', '.join(blacklisted)}"
                                    if blacklisted
                                    else "No blacklisted premises"
                                ),
                                size=16,
                            ),
                        ],
    ...
    page.add(ft.Column([tabs, tab_content]))
